...
df = pd.read_csv('KakaoTalkChats.txt', delimiter = '\t')

### 단톡방 인원수 pcp(participant) 변수에 빼내기
for col in df.columns:
    origin = col
    pcp = col
pcp = int(pcp.split('(')[1].split(')')[0]) # 단톡방 인원수
...
yeardf = df1['year'].value_counts(sort=False).rename_axis('year').to_frame('counts')
...
userdf = userdf.take(userdf.index[0:pcp]) # 단톡방 참여자 수 제외하고 전부 드랍
...
for user in users:
    ...
    while True:
        if date == last:
            break
        condition = (df2['user'] == user) & (df2['datetime'] == date) # 조건식 작성
        ser[user] += df2[condition].value_counts().sum()
        date = datetime.datetime.strptime(date,'%Y-%m-%d')
        date += datetime.timedelta(days=1)
        date = date.strftime('%Y-%m-%d')
        
    ser[user] = round(ser[user]/365, 2)
    series = pd.concat([series, ser], join='outer')
